...
2.5. **Auto-Apply Analysis Remediation (if `analysis.md` exists)**:
   - Check if `FEATURE_DIR/analysis.md` exists.
   - If it exists, scan for checked items under `## 4. Remediation Action Checklist`:
     - If any items are checked `- [x] R...`:
       - Parse the remediation action and apply the recommended fixes directly to `spec.md`, `plan.md`, or `tasks.md` before executing implementation.
       - Log in output: `[Remediation] Applied fix: <Action description>`.
       - Mark the remediation in `analysis.md` as completed.
     - If zero findings or no checked items, proceed directly without modifying artifacts.
